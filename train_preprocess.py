...
def fold_train_idx(pos_folds, nFold, foldID):
    fold_idx = []
    for fold in range(nFold):
        if fold == foldID:
            continue
        fold_idx.append(pos_folds['fold_' + str(fold)])
    fold_idx = np.concatenate(fold_idx, axis=1)
    return fold_idx
...
if __name__ == '__main__':
    ...
    for counter in range(num_repeats):  # repeat ten times
        ...
        for foldID in range(nFold):
            print('\nThis is Fold ', str(foldID), '...')
            fold_drug_protein = np.zeros_like(drug_protein)
            fold_idx = fold_train_idx(pos_folds, nFold, foldID)
            fold_drug_protein[fold_idx[0], fold_idx[1]] = drug_protein[fold_idx[0], fold_idx[1]]

            ## Syep 2: Build the Adjacency Matrix
            # Drug-0, Protein-1, Disease-2, Side-effect-3
            adjM = get_adjM(drug_drug, fold_drug_protein, drug_disease, drug_sideEffect, protein_protein, protein_disease,
                            num_drug, num_protein, num_disease, num_se)
            # sp.save_npz(save_prefix + 'adjM_train_fold_{}.npz'.format(foldID), sp.csr_matrix(adjM))
            ...
            drug_disease_lit = {i: adjM[i, num_drug + num_protein:num_drug + num_protein + num_disease].nonzero()[0] for i
                                in range(num_drug)}
            ...
            protein_protein_list = {i: adjM[num_drug + i, num_drug:num_drug + num_protein].nonzero()[0] for i in
                                    range(num_protein)}
            protein_disease_list = {
                i: adjM[num_drug + i, num_drug + num_protein:num_drug + num_protein + num_disease].nonzero()[0]
                for i in range(num_protein)}
            disease_drug_list = {i: adjM[num_drug + num_protein + i, :num_drug].nonzero()[0] for i in range(num_disease)}
            disease_protein_list = {i: adjM[num_drug + num_protein + i, num_drug:num_drug + num_protein].nonzero()[0] for i
                                    in range(num_disease)}
            se_drug_list = {i: adjM[num_drug + num_protein + num_disease + i, : num_drug].nonzero()[0] for i in range(num_se)}

            # Step 3: Get target metapaths and neighbors for each train fold
            ...
            for i, metapaths in enumerate(expected_metapaths):
                ...
                for metapath in metapaths:
                    metapath_fold_dir = save_prefix + 'repeat{}/{}/train/fold_{}/_'.format(counter, i, foldID)
                    make_dir(metapath_fold_dir)
                    # Get all the metapaths in the schema of 'metapath'
                    if os.path.isfile(metapath_fold_dir + '-'.join(map(str, metapath)) + '.npy'):
                        edge_metapath_idx_array = np.load(metapath_fold_dir + '-'.join(map(str, metapath)) + '.npy')
                    else:
                        edge_metapath_idx_array = get_metapath(metapath, num_drug, num_protein, num_disease, num_se, metapath_fold_dir)
                        np.save(metapath_fold_dir + '-'.join(map(str, metapath)) + '.npy', edge_metapath_idx_array)
                    ...
                    pickle.dump(target_metapaths, open(metapath_fold_dir + '-'.join(map(str, metapath)) + '.idx.pkl', 'wb'))
                    pickle.dump(target_neighbors, open(metapath_fold_dir + '-'.join(map(str, metapath)) + '.adjlist.pkl', 'wb'))
